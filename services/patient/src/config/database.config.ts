...
export function getDatabaseConfig(): DatabaseConfig {
  return {
    ...
    port: Number(process.env.DB_PORT ?? 5549),
    user: process.env.DB_USER ?? 'patient_service',
    password: process.env.DB_PASSWORD ?? 'patient_service',
    database: process.env.DB_NAME ?? 'patient_service',
  };
}
