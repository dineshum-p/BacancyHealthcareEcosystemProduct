 ...
 * full scope boundary). `PatientsModule` imports this and injects
 * `DOMAIN_EVENT_PUBLISHER` into `PatientsService` so `POST /patients`
 * always publishes `patient.created` (BAC-14, AC4) through the SAME code
 * path regardless of which concrete transport is bound.
 */
@Module({
  providers: [
    {
