 ...
 * `PatientsService.create` always exercises, logging the event instead of
 * publishing it onto a real broker, so BAC-14 AC4's "emits a patient.created
 * domain event" requirement is satisfied by the wiring being correct and
 * unconditional, not by a real broker being reachable in this environment.
 */
@Injectable()
export class NoopDomainEventPublisher implements DomainEventPublisher {
  ...
  publishPatientCreated(event: PatientCreatedEvent): Promise<void> {
    this.logger.log(
      `[patient.created] (KAFKA_PRODUCER_ENABLED=false, no real broker configured) ${JSON.stringify(event)}`,
    );
