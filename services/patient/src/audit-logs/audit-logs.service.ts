 ...
 * `AuditLogsService` (and `services/emr`'s/`services/billing`'s copies):
 * `record` is called by `AuditLogInterceptor` after every audited mutation.
 * Always takes an explicit `schemaName` -- resolved by the caller via
