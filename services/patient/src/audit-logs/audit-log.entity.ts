 ...
 * `AuditLogEntry` (and `services/emr`'s/`services/billing`'s copies)
 * exactly. `before`/`after` are `unknown` because they hold whatever shape
 * the audited resource's response body has -- here always a
 * `PatientResponseDto`, but deliberately not typed as such, since the whole
 * point of the interceptor/decorator mechanism is to stay generic across
 * resource types.
 */
export interface AuditLogEntry {
   ...
   * access-token identity. In practice every `/patients` mutation route in
   * this service is guarded by `AccessTokenGuard` (BAC-14), so this is
