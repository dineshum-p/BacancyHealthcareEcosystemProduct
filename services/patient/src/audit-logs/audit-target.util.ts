 ...
 * `services/tenant`'s BAC-8 `resolveAuditAction` (and `services/emr`'s/
 * `services/billing`'s copies) exactly: records the semantic name (e.g.
 * `'create'` instead of `'POST'`) so a future audit entry reads the same way
 * regardless of which verb a given API design chose.
 */
 ...
 * `services/tenant`'s BAC-8 version: every `/patients` route in this
 * service is guarded by `TenantGuard` (BAC-14), so `request.tenant` is
