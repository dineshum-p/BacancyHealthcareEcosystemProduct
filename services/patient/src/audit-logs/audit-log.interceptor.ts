 ...
 * package: `services/tenant`'s copy is wired to ITS OWN
 * `TenantSchemaProvisioner`/`RequestWithAuth`/`AuditLogsService`, none of
 * which this independently deployable service can import as TypeScript --
 * exactly the same reasoning `TenantGuard`/`AccessTokenGuard` are
 * duplicated per-service throughout this repo (`services/emr`'s BAC-10 and
 * `services/billing`'s BAC-11 copies took the identical approach). This is
 * this ticket's documented answer to CLAUDE.md's "write an audit log entry
 * per BAC-8's existing pattern" instruction: the MECHANISM (interceptor +
 * decorator + append-only repository, with the exact same DDL and
 * behavioural contract) is reused verbatim; only the wiring (this service's
 * own schema provisioner/tenant-context types) is service-local, because
 * BAC-8 was never built as a cross-service-importable package.
 *
