...
`services/notification`'s, `services/emr`'s, and `services/billing`'s are
(see those services' `migrations/README.md`):
...
- `public.tenants` (the shared tenant registry) is owned and migrated by
  ...
  tenant.
- This service's own domain tables -- `<tenant_schema>.patients`,
  `<tenant_schema>.patient_mrn_counters` (BAC-14), and
  `<tenant_schema>.audit_logs` (mirroring `services/tenant`'s BAC-8
  mechanism) -- live inside each tenant's dynamically provisioned Postgres
  schema, one per tenant, and can't be expressed as a single static
  migration. They are created lazily and idempotently by
  `PatientSchemaProvisioner` (`src/patients/patient-schema.provisioner.ts`)
  the first time this process needs them for a given tenant -- the same
  pattern `EmrSchemaProvisioner`/`BillingSchemaProvisioner`/
