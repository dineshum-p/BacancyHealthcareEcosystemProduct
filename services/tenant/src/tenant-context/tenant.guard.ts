...
/**
 * Resolves tenant context for every guarded route (AC1) and rejects
 * unknown/inactive tenants before any handler or repository ever touches a
 * database connection (AC2). Runs as a Nest guard (not middleware) so
 * thrown HttpExceptions are handled by Nest's exception filters.
 */
@Injectable()
