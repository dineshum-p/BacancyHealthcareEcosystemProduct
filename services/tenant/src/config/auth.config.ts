...
/**
 * Fail-fast guard mirroring `services/auth`'s `auth.config.ts`
 * (`assertSecretConfiguredOutsideDevTest`): refuses to let an unset/blank/
 ...
 * in this project (BAC-5, BAC-6) -- `services/tenant` must not reintroduce it
 * now that it verifies JWTs for the first time (BAC-8).
 *
 * `services/tenant` never SIGNS tokens (only `services/auth` does), but it
 * must use the exact same secret to verify them, so the placeholder here is
 * intentionally identical to `services/auth`'s -- both services must be
