...
/**
 * `services/tenant` never signs tokens (only `services/auth` does), so these
 * tests sign with a plain `JwtService` standing in for `services/auth`'s own
 * `AccessTokenService.sign` -- the point being proven is that THIS service's
 * verify-only `AccessTokenService` independently verifies a token signed
 * elsewhere, using the same secret/algorithm convention (BAC-8).
 */
