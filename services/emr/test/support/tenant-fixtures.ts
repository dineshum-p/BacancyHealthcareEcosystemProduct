 ...
 * done in production before `services/emr` ever sees the tenant.
 * `services/emr` itself never creates schemas (only its own
 * `patients`/`audit_logs` tables inside an existing one).
 */
