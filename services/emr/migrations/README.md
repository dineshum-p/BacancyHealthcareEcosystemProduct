...
- `public.tenants` (the shared tenant registry) is owned and migrated by
  ...
  tenant.
- This service's own domain tables -- `<tenant_schema>.patients` (BAC-10) and
  `<tenant_schema>.audit_logs` (mirroring `services/tenant`'s BAC-8 mechanism)
  -- live inside each tenant's dynamically provisioned Postgres schema, one
  per tenant, and can't be expressed as a single static migration. They are
  created lazily and idempotently by `EmrSchemaProvisioner`
  (`src/fhir/emr-schema.provisioner.ts`) the first time this process needs
  them for a given tenant -- the same pattern `NotificationsSchemaProvisioner`
  uses in `services/notification` and `AuthSchemaProvisioner` uses in
  `services/auth`.
