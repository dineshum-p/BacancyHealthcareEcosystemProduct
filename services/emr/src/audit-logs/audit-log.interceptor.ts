 ...
 * activated per-route by `@Audited(resourceType)` -- here, `@Audited('Patient')`
 * on `PatientsController.create()`.
 *
 * Duplicated (not literally imported) rather than a shared `@hep/*` package:
 * `services/tenant`'s copy is wired to ITS OWN `TenantSchemaProvisioner`/
 * `RequestWithAuth`/`AuditLogsService`, none of which this independently
 * deployable service can import as TypeScript -- exactly the same reasoning
 * `TenantGuard`/`AccessTokenGuard` are duplicated per-service throughout
 * this repo. This is this ticket's documented answer to CLAUDE.md's "reuse
 * the existing audit-log mechanism from BAC-8" instruction: the MECHANISM
 * (interceptor + decorator + append-only repository, with the exact same
 * DDL and behavioural contract) is reused verbatim; only the wiring (this
 * service's own schema provisioner/tenant-context types) is service-local,
 * because BAC-8 was never built as a cross-service-importable package.
 *
