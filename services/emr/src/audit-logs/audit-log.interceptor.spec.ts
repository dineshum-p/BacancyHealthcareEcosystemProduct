...
describe('AuditLogInterceptor', () => {
  ...
  it('passes the request through untouched when no @Audited metadata is present', async () => {
    ...
    const handler = makeHandler({ resourceType: 'Patient', id: '1' });

    const result = await firstValueFrom(
      interceptor.intercept(context, handler),
    );

    expect(result).toEqual({ resourceType: 'Patient', id: '1' });
    // eslint-disable-next-line @typescript-eslint/unbound-method -- jest.fn() mock
    expect(auditLogsService.record).not.toHaveBeenCalled();
  });

  it('records a create with before=null, after=<FHIR resource>, actorUserId from request.user (BAC-10, AC2)', async () => {
    const metadata: AuditedMetadata = { resourceType: 'Patient', options: {} };
    ...
    const after = { resourceType: 'Patient', id: 'patient-1' };
  ...
  it('throws (failing the request) when no tenant schema can be resolved for the audit entry', async () => {
    ...
    reflector.get.mockReturnValue(metadata);
    const handler = makeHandler({ resourceType: 'Patient', id: 'p1' });
