...
export const AUDITED_METADATA_KEY = 'bac10:audited';

/**
 * Options for `@Audited(...)`, mirroring `services/tenant`'s BAC-8
 * mechanism exactly. `resolveBefore` is the contract point a future
 * PUT/PATCH/DELETE handler on `/fhir/Patient/:id` would need; `POST
 * /fhir/Patient` (this ticket's only mutation) is a creation, which has no
 * "before" state by definition, so it does not supply one.
 */
