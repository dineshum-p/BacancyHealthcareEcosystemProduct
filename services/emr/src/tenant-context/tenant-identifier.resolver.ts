 ...
 * `X-Tenant-Id` header takes priority, falling back to the first label of
 * the Host header when it looks like a tenant subdomain (e.g.
 * `acme.app.example.com` -> `acme`). Mirrors `services/tenant`'s resolver
 * (BAC-4) exactly. Returns null when neither source yields a usable
 * identifier.
 */
