 ...
 * may act on ANY patient's resource; a `patient` may only ever act on their
 * OWN (`targetPatientId` must equal their own `userId`).
 *
 * Deliberately generic/reusable rather than hardcoded to one entity type:
 * this service has TWO patient-owned resource domains (`fhir/Patient` and
 * `encounters`), and both would reuse the SAME two functions -- the caller
 * supplies whichever id the resource in question uses as its owning-patient
 * field, and this utility only ever compares it against the caller's
 * `userId`. This ticket (BAC-41) only adds this utility as a foundation; no
 * controller/service wires it in yet -- a follow-on ticket (e.g. BAC-44) is
 * expected to be the first real caller, once `role-permissions.map.ts`
