 ...
 * `services/notification`'s `auth.config.ts`: refuses to let an unset/blank/
 ...
 *
 * `services/emr` never SIGNS tokens (only `services/auth` does), but it must
 * use the exact same secret to verify them, so the placeholder here is
