...
export function getDatabaseConfig(): DatabaseConfig {
  return {
    ...
    port: Number(process.env.DB_PORT ?? 5547),
    user: process.env.DB_USER ?? 'emr_service',
    password: process.env.DB_PASSWORD ?? 'emr_service',
    database: process.env.DB_NAME ?? 'emr_service',
  };
}
