...
describe('getEventPublisherConfig', () => {
  ...
  it('defaults to disabled with sensible broker/topic defaults', () => {
    ...
    delete process.env.KAFKA_ENCOUNTER_CREATED_TOPIC;
  ...
  it('reads a topic override from the environment', () => {
    process.env.KAFKA_ENCOUNTER_CREATED_TOPIC = 'custom.encounter.created';
    expect(getEventPublisherConfig().encounterCreatedTopic).toBe(
      'custom.encounter.created',
    );
  });
});
