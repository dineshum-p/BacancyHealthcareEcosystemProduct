 ...
 * `services/scheduling` never provisions or mutates tenants -- that's
 ...
 * schema to bind appointment queries to. Mirrors `services/patient`'s/
 * `services/billing`'s/`services/emr`'s `TenantsRepository` exactly.
 */
@Injectable()
