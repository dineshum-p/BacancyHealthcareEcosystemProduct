 ...
 * user identity -- mirrors every other service's `AccessTokenGuard` exactly,
 * since every service must agree on the same verification semantics for a
 * token to mean the same thing wherever it is checked.
 *
 * BAC-16: every `/appointments/*` route requires a valid JWT AND a resolved,
