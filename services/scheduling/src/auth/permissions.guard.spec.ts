...
describe('PermissionsGuard', () => {
  ...
  it('throws 403 when the role lacks the required permission (e.g. an unrecognized role)', () => {
    const context = makeContext('guarded', {
      // Every REAL UserRole is granted MANAGE_APPOINTMENTS in this service
      // (see role-permissions.map.ts's doc comment: the ticket's real RBAC
      // differentiator is instance-level, not role-level) -- an unrecognized
      // role string exercises the 403 branch via ROLE_PERMISSIONS' `?? []`
      // fallback.
      user: { userId: 'u1', tenantId: 't1', role: 'unknown_role' as never },
    });
