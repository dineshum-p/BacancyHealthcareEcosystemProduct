 ...
 * `services/tenant`'s BAC-8 `AuditLogsRepository` (and every other
 * service's copy) -- the read/`GET /audit-logs` side is out of this ticket's
 * scope, so only `insert` is ported. Append-only: this class deliberately
 * has no `update`/`delete` method.
 */
@Injectable()
