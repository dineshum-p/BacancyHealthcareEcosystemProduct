 ...
 * done in production before `services/scheduling` ever sees the tenant.
 * `services/scheduling` itself never creates schemas (only its own
 * `appointments`/`audit_logs` tables inside an existing one).
 */
