 ...
 *
 * Mirrors `services/tenant`'s, `services/auth`'s, `services/notification`'s,
 * and `services/emr`'s `test/support/create-in-memory-pool.ts`. Own copy
 * because `services/billing` is an independently deployable app and does not
 * import TypeScript from any of them.
 *
