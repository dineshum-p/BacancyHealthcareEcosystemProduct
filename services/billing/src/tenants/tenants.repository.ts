 ...
 * `services/billing` never provisions or mutates tenants -- that's
 * `services/tenant`'s job -- it only resolves a tenant by identifier to
 * enforce tenant scoping (reject unknown/inactive tenants), to know which
 * schema to bind usage-event queries to, and which `plan` (BAC-11, AC4)
 * defines that tenant's metered limits. Mirrors `services/emr`'s
