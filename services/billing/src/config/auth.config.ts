 ...
 * `services/notification`/`services/emr`'s `auth.config.ts`: refuses to let
 * an unset/blank/known-placeholder secret stand in for a real one outside
 * `test`/`development` (i.e. anything that could be a real deployment,
 * including an unset `NODE_ENV`). This exact class of config vulnerability
 * (an insecure default silently accepted in production) was found and fixed
 * twice already in this project (BAC-5, BAC-6) -- every new service that
 * verifies JWTs must not reintroduce it.
 *
 * `services/billing` never SIGNS tokens (only `services/auth` does), but it
