 ...
 * `services/billing` is an independently deployable NestJS app with its own
 ...
 * approach `services/emr` (BAC-10) already established when it became the
 * first service after `services/auth` itself to reuse the
 * `PermissionsGuard` mechanism).
 */
