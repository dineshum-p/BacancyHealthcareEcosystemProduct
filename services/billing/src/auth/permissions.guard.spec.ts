...
describe('PermissionsGuard', () => {
  ...
  it('allows a role that holds the required permission through (AC1 -- RECORD_USAGE for STAFF)', () => {
    const reflector = makeReflector([Permission.RECORD_USAGE]);
    const guard = new PermissionsGuard(reflector);
    const request: Partial<RequestWithAuth> = {
  ...
  it('rejects a role lacking the required permission with 403, not 401 (AC4 -- READ_USAGE for STAFF)', () => {
    const reflector = makeReflector([Permission.READ_USAGE]);
    const guard = new PermissionsGuard(reflector);
    const request: Partial<RequestWithAuth> = {
      user: { userId: 'staff-1', tenantId: 't1', role: UserRole.STAFF },
    };
  ...
  it('allows a CLINIC_ADMIN through for READ_USAGE', () => {
    const reflector = makeReflector([Permission.READ_USAGE]);
    const guard = new PermissionsGuard(reflector);
    const request: Partial<RequestWithAuth> = {
      user: { userId: 'admin-1', tenantId: 't1', role: UserRole.CLINIC_ADMIN },
    };
