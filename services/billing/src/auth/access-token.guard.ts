 ...
 * `services/emr`'s `AccessTokenGuard` exactly, since every service must
 * agree on the same verification semantics for a token to mean the same
 * thing wherever it is checked.
 *
 * BAC-11: every `/billing/*` route requires a valid JWT AND a resolved,
