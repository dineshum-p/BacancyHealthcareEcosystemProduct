...
/**
 * Authorization guard (BAC-11, AC1/AC4), reusing `services/auth`'s BAC-7
 ...
 * `services/emr`'s BAC-10 copy): reads the `role` claim from the
 * already-validated access token (`request.user`, set by
 * `AccessTokenGuard`) and checks it against `@RequirePermissions(...)`'s
...
export class PermissionsGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    // `services/auth`'s/`services/emr`'s `PermissionsGuard` uses.
