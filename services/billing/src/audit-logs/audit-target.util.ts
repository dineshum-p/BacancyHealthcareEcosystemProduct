 ...
 * BAC-10 copy) exactly: records the semantic name (e.g. `'create'` instead
 * of `'POST'`) so a future audit entry reads the same way regardless of
 * which verb a given API design chose.
 */
 ...
 * `services/tenant`'s BAC-8 version: every `/billing/*` route in this
 * service is guarded by `TenantGuard` (BAC-11), so `request.tenant` is
