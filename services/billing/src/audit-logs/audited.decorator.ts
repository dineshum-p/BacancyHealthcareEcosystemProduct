...
export const AUDITED_METADATA_KEY = 'bac11:audited';

/**
 * Options for `@Audited(...)`, mirroring `services/tenant`'s BAC-8 (and
 * `services/emr`'s BAC-10) mechanism exactly. `resolveBefore` is the
 * contract point a future PUT/PATCH/DELETE handler would need; `POST
 * /billing/usage/events` (this ticket's only mutation) is a creation, which
 * has no "before" state by definition, so it does not supply one.
 */
