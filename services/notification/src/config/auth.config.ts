...
/**
 * Fail-fast guard mirroring `services/tenant`'s `auth.config.ts` (itself
 * mirroring `services/auth`'s `assertSecretConfiguredOutsideDevTest`):
 * refuses to let an unset/blank/known-placeholder secret stand in for a real
 * one outside `test`/`development` (i.e. anything that could be a real
 * deployment, including an unset `NODE_ENV`). This exact class of config
 * vulnerability (an insecure default silently accepted in production) was
 * found and fixed twice already in this project (BAC-5, BAC-6) -- every new
 * service that verifies JWTs must not reintroduce it.
 *
 * `services/notification` never SIGNS tokens (only `services/auth` does),
 * but it must use the exact same secret to verify them, so the placeholder
 * here is intentionally identical to `services/auth`'s/`services/tenant`'s --
 * all three services must be configured with the same real
 * `JWT_ACCESS_SECRET` in any real deployment.
 */
