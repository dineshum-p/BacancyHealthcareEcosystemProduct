 ...
 * `services/notification` never provisions or mutates tenants -- that's
 ...
 * schema to bind `notifications` queries to. Mirrors `services/auth`'s
 * `TenantsRepository` exactly.
 */
@Injectable()
export class TenantsRepository {
  ...
  /**
   * Resolves a tenant strictly by id -- used by the domain-event consumption
   * path (AC4), where the event payload carries a `tenantId`, not an
   * `X-Tenant-Id` header/slug.
   */
